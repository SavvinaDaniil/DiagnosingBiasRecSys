...
from lenskit import util, batch, Recommender
...
def optimize_cornac(ratings, algorithm_name, args, max_evals, verbose=False):
    ...
    ratio_split = RatioSplit(
        data=ratings.values, test_size=0.2, val_size=0.2, verbose=verbose, seed=0
    )
...
def optimize_lkpy(ratings, algorithm_name, args, partition_way, max_evals):
    def objective_mf(params):
        ...
        algorithm = als.BiasedMF(
            features=features, reg=reg, iterations=iterations, bias=bias, rng_spec=0
        )
    ...
    def objective_bpr(params):
        ...
        algorithm = lenskit_tf.BPR(
            features=features, reg=reg, epochs=epochs, rng_spec=0
        )
        fittable = util.clone(algorithm)
        fittable = Recommender.adapt(fittable)
        fittable.fit(train_df)
    ...
    if algorithm_name == "UserKNN":
        ...
        space = {
            "features": hp.choice("features", [10, 50, 100]),
    ...
    trials = Trials()
    best_params = fmin(
        obj_function, space, algo=tpe.suggest, max_evals=max_evals, trials=trials
    )
